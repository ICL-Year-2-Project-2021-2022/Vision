...
COM_COUNTER_SECTION dut(
    ...
    .sink_data(sink_data), .sink_valid(sink_valid), .sink_sop(sink_sop), .sink_eop(sink_eop),
    .source_data(source_data), .source_valid(source_valid), .source_ready(source_ready), .source_sop(source_sop), .source_eop(source_eop),
    .s_chipselect(s_chipselect), .s_read(s_read), .s_write(s_write), .s_readdata(s_readdata), .s_writedata(s_writedata), .s_address(s_address),
    .ex_raw(ex_raw), .ey_raw(ey_raw), .mass(mass)
);
