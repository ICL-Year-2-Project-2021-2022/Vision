...
void A_star(int start_x, int start_y, int goal_x, int goal_y, int grid[5][5]){
    Node* open = newNode(0, 0, 5, 5,distanceToGoal(start_x, start_y, goal_x, goal_y),  NULL);//start at (0,0)
    Node* close = NULL;
    ...
    while (1){
        ...
        for (int i=-1; i<=1; i++){
            for (int j=-1; j<=1; j++){
                ...
                Node* neighbour_node = newNode(neighbour_x, neighbour_y, distanceStart, priority, current);
